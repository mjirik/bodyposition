...
def prepare_data(
    ...
    for i in range(n_data):
        ...
        X_train = np.empty([len(data), imshape, imshape], dtype=np.float)
        ...
        if not skip_h5:
            if organ_detection_on:
                ...
                with h5py.File(f'{filename_prefix}sdf_liver{imshape}.h5', 'a') as h5f:
                    logger.debug(f"X_train={X_train.dtype}")
                    h5f.create_dataset(f'scan_{i}', data=X_train)
                    h5f.create_dataset(f'label_{i}', data=Y_train3)
                with h5py.File(f'{filename_prefix}sdf_spleen{imshape}.h5', 'a') as h5f:
                    logger.debug(f"X_train={X_train.dtype}")
                    h5f.create_dataset(f'scan_{i}', data=X_train)
                    h5f.create_dataset(f'label_{i}', data=Y_train4)
                with h5py.File(f'{filename_prefix}sdf_lungs{imshape}.h5', 'a') as h5f:
                    logger.debug(f"X_train={X_train.dtype}")
                    h5f.create_dataset(f'scan_{i}', data=X_train)
                    h5f.create_dataset(f'label_{i}', data=Y_train5)
            else:
